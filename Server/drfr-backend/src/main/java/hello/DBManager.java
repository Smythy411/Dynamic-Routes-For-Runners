 ...
 * Many of these methods are no longer used, as better more robust versions have been developed.
 * However I have left them here for legacy purposes
 */
...
public class DBManager {
 ...
 //Gets Edges by bounding box using an arraylist of OSMNodes *LEGACY*
   ...
   //Gets Edges by bounding box using a Map of OSMNodes
   ...
   //Gets the relevant nodes by 'wayID'
   public ArrayList<OSMEdge> getEdgesbyWayId(long wayID) {
	   ArrayList<OSMEdge> edges = new ArrayList<>();
	   try {
		   Statement stmt = c.createStatement();
		   ResultSet rs = stmt.executeQuery("SELECT * FROM edgelist WHERE wayID = " + wayID + ";");
		   while(rs.next()) {
			   OSMNode source = getNodeByNodeId(rs.getLong("sourceNode"));
			   OSMNode target = getNodeByNodeId(rs.getLong("targetNode"));
			   OSMEdge edge = new OSMEdge(rs.getLong("edgeID"), rs.getLong("wayID"), source, target);
			   source.addEdge(edge);
			   target.addEdge(edge);
